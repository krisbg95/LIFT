  ...
  for(z in 1:nrepeats){
    ...
    for (i in 1:ncv_folds[1]){  
      ...
      atts <- foreach(j=1:ncv_folds[2],.packages = c('foreach', 'caret',"DMwR2","dplyr",'CORElearn','RGCCA',"kernlab","gbm","modelgrid","stats","mice",'tidyverse')) %dopar% {  
        ...
        ## Subset features to FC and outcome
        x1 = train_cca %>% dplyr::select(starts_with(functional)) 
        x5 = train_cca[,label] 
        ...
        for(l in 1:100){
          ...
          names(variableset) = "Functional"
          variables[[l]] = variableset
        }
        
        variableset1 = lapply(variables, `[`, "Functional")
...
save(finalsave,file = paste0(getwd(),"/Sequential_FC.RData"))