  ...
  for(z in 1:nrepeats){
    ...
    for (i in 1:ncv_folds[1]){  
      ...
      atts <- foreach(j=1:ncv_folds[2],.packages = c('foreach', 'caret',"DMwR2","dplyr",'CORElearn','RGCCA',"kernlab","gbm","modelgrid","stats","mice",'tidyverse')) %dopar% {  
        ...
        ## Complete case and k-mean imputation of missing clinical variables
        train_cca = train_cca[complete.cases(train_cca[,label]),] 
        train_cca = DMwR2::knnImputation(train_cca)   
        
        ## Subset features to Clinical and outcome
        ...
        ## Function to calculate parameter based on number of variables
        shrink = function(a){
        ...
        mltest = DMwR2::knnImputation(mltest,distData = mltrain)
        mltrain = DMwR2::knnImputation(mltrain) 
      ...
      ## Create same model using only baseline fatigue 
...
for (v in 1:nrow(f_sel)){names(finalf)[v] = paste(f_sel[v,]$relief,f_sel[v,]$stability,f_sel[v,]$fdata,f_sel[v,]$ddata)}
methods = algor
suffix = c("Median","IQR")
names = paste(rep(methods, each = length(suffix)), suffix, sep = "")
summary = as.data.frame(matrix(nrow=nrow(f_sel), ncol = length(names)))
names(summary) = names
for (v in 1:nrow(f_sel)){rownames(summary)[v] = paste(f_sel[v,]$relief,f_sel[v,]$stability,f_sel[v,]$fdata,f_sel[v,]$ddata)}
...
save(finalsave,file = "Sequential_Clinical.RData")
