  ...
  for(z in 1:nrepeats){
    ...
    for (i in 1:ncv_folds[1]){  
      ...
      atts <- foreach(j=1:ncv_folds[2],.packages = c('foreach', 'caret',"DMwR2","dplyr",'CORElearn','RGCCA',"kernlab","gbm","modelgrid","stats","mice",'tidyverse')) %dopar% {  
        ...
        ## Subset features to SC and outcome
        x1 = train_cca %>% dplyr::select(starts_with("SC1_"))   
        x5 = train_cca[,label] 
        ...
        for(l in 1:100){
          ...
          names(variableset) = "DTI"
          variables[[l]] = variableset
        }
        
        variableset1 = lapply(variables, `[`, "DTI")
      ...
      model_sel = function(a){
        ...
        # selected_v = selected_v[-length(selected_v)]
...
save(finalsave,file = paste0(getwd(),"/Sequential_SC.RData"))
