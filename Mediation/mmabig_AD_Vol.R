...
write.table(data.frame(colnames(frmsig)), 'med_listvol.csv', append=T, sep=',')
write.table(data.frame(frmsig), 'volsigtotal.csv', append=T, sep=',')
# write.table(data.frame(medheader), 'featurenames.csv', append=T, sep=',')

# plot(mma.e1,vari="LH.Lateral.Occipital.Gyrus")
save.image("./workspace_Volume.RData")
