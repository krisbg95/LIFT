...
write.table(data.frame(colnames(frmsig)), 'med_listrsFC.csv', append=T, sep=',')
write.table(data.frame(frmsig), 'rsFCsigtotal.csv', append=T, sep=',')
write.table(data.frame(medheader), 'featurenames.csv', append=T, sep=',')

save.image("./workspace_rsFC.RData")
