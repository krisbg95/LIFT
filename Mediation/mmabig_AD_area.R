...
medvars <- read.table("./varsarea.txt", quote="\"", comment.char="")
...
write.table(data.frame(colnames(frmsig)), 'med_listarea.csv', append=T, sep=',')
write.table(data.frame(frmsig), 'areasigtotal.csv', append=T, sep=',')
# write.table(data.frame(medheader), 'featurenames.csv', append=T, sep=',')

save.image("./workspace_area.RData")
